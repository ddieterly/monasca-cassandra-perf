...
public class MPerfThreadedMultiTableWithMetricBucketPartitions {

  private static final Timestamp BUCKET_START_TIMESTAMP = new Timestamp(0);
  ...
  private static final Logger logger = LoggerFactory.getLogger(
      MPerfThreadedMultiTableWithMetricBucketPartitions.class);
  ...
  public static void main(String[] args) {
    ...
    if (args.length != 4) {
      System.out.println("Usage: MPerfThreadedMultiTable <num threads> <num measurements per thread> <num unique metrics> <num metric partitions");
      System.exit(-1);
    }
    ...
    int numMetricPartitions = Integer.parseInt(args[3]);
    ...
    MPerfThreadedMultiTableWithMetricBucketPartitions
        mPerfThreaded = new MPerfThreadedMultiTableWithMetricBucketPartitions(numThreads, numMeasurementsToInsert, numUniqueMetrics, numMetricPartitions);
  ...
  public MPerfThreadedMultiTableWithMetricBucketPartitions(int numThreads, int numMeasurementsToInsert,
                                                           int numUniqueMetrics, int numMetricPartitions) {
    ...
    for (int i = 0; i < numThreads; i++) {
      ...
      mPerfRunnableList.add(new MPerfRunnable(numMeasurementsToInsert, numUniqueMetrics,
                                              numMetricPartitions, String.valueOf(i)));
  ...
  private static class MPerfRunnable implements Runnable {
    ...
    private final int numMetricPartitions;
    ...
    MPerfRunnable(int numMeasurementsToInsert, int numUniqueMetrics, int numMetricPartitions,
                  String threadId) {

      this.numMeasurmentsToInsert = numMeasurementsToInsert;
      this.numUniqueMetrics = numUniqueMetrics;
      this.numMetricPartitions = numMetricPartitions;
      ...
      this.metricsInsertStmt =
          session.prepare(
              "INSERT INTO monasca.metrics (region, tenant_id, bucket, created_at, "
    ...
    public void run() {
      ...
      try {
        ...
        for (int i = 0; i < numMeasurmentsToInsert; i++) {
          ...
          Timestamp updatedAt = new Timestamp(calendar.getTime().getTime());
          ...
          int metricPartitionNum = i % this.numMetricPartitions;

          BoundStatement
              metricBoundStmt =
              metricsInsertStmt
                  .bind(REGION, TENANT_ID, metricPartitionNum, createdAt, updatedAt, metricIdSha1HashByteBuffer, metricName, dimensionHashString);
          ...
          if (i % 10000 == 0) logger.debug("Thread {} has {} left", this.threadId, numMeasurmentsToInsert - i);
